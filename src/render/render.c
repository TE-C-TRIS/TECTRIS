...
void DrawGame(GameContext *ctx, int menuIndex) {
    ...
    if (ctx->state == STATE_MENU)
    {
        ...
        int sh = ctx->screen.screenHeight;

        DrawRectangle(0, 0, sw, sh, (Color){0, 0, 0, 220});

        DrawText(
            "TECTRIS",
            sw / 2 - MeasureText("TECTRIS", (int)(70 * s)) / 2,
            (int)(sh * 0.30f),
            (int)(70 * s),
            COLOR_TEXT
        );
        ...
        DrawText(
            "Aprenda C Jogando",
            sw / 2 - MeasureText("Aprenda C Jogando", (int)(25 * s)) / 2,
            (int)(sh * 0.42f),
            (int)(25 * s),
            WHITE
        );
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            DrawText(
                options[i],
                sw / 2 - MeasureText(options[i], (int)(30 * s)) / 2,
                (int)(sh * 0.55f + i * 40 * s),
                (int)(30 * s),
                textColor
            );
    ...
    Rectangle boardRect = {
        off.x,
        off.y,
        (float)BOARD_WIDTH * cs,
        (float)BOARD_HEIGHT * cs
    };
    ...
    DrawLineEx(
        (Vector2){off.x, (float)killY},
        (Vector2){off.x + BOARD_WIDTH * cs, (float)killY},
        4 * s,
        kColor
    );
    ...
    char scoreStr[16];
    sprintf(scoreStr, "%06d", ctx->score);
    ...
    for (int y = 0; y < ctx->nextPiece.size; y++) {
        ...
        for (int x = 0; x < ctx->nextPiece.size; x++) {
            ...
            if (ctx->nextPiece.shape[y][x]) {
                ...
                DrawRectangle(
                    panelX + (int)(50 * s) + x * nextCs,
                    (int)(off.y + 350 * s) + y * nextCs,
                    nextCs - 2,
                    nextCs - 2,
                    ctx->nextPiece.color
                );
